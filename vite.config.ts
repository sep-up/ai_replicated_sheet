...
export default defineConfig(({ mode }) => {
  const { VITE_PUBLIC_PATH, VITE_NGINX_PATH, VITE_BASE_API } = loadEnv(mode, process.cwd(), '') as ImportMetaEnv
  return {
    // 开发或打包构建时用到的公共基础路径
        ...
        targets: [
          {
            ...
            transform: (content) => {
              ...
              fs.writeFile(path.join(__dirname, './AppManifest.json'), JSON.stringify(manifest))
